...
def best_match(tex, over, mask, xi, xf, yi, yf):
    Area1 = np.copy(tex[:, :320 - 1])  # ok
    Area2 = np.copy(tex[215:, :820 - 1])  # ok
    Area3 = np.copy(tex[215:605 - 1, :1270])  # ok
    Area4 = np.copy(tex[215:, 575:820 - 1])  # ok
    Area5 = np.copy(tex[215:605 - 1, 575:950])  # ok
    Area6 = np.copy(tex[320:, 1270:])  # ok
    Area7 = np.copy(tex[:605 - 1, 1030:1270])  # ok
    ...
    corr5 = cv2.matchTemplate(Area5, over, cv2.TM_CCORR_NORMED, mask=mask)
    corr6 = cv2.matchTemplate(Area6, over, cv2.TM_CCORR_NORMED, mask=mask)
    corr7 = cv2.matchTemplate(Area7, over, cv2.TM_CCORR_NORMED, mask=mask)
    ...
    for i in range(0, num):
        ...
        minVal5, maxVal5, minLoc5, maxLoc5 = cv2.minMaxLoc(corr5)
        minVal6, maxVal6, minLoc6, maxLoc6 = cv2.minMaxLoc(corr6)
        minVal7, maxVal7, minLoc7, maxLoc7 = cv2.minMaxLoc(corr7)
        ...
        corr5[maxLoc5[1], maxLoc5[0]] = -100
        corr6[maxLoc6[1], maxLoc6[0]] = -100
        corr7[maxLoc7[1], maxLoc7[0]] = -100
    ...
    maxVal5 = h.pop(random.randint(0, num - 1))
    maxVal6 = f.pop(random.randint(0, num - 1))
    maxVal7 = g.pop(random.randint(0, num - 1))
    e = [maxVal1, maxVal2, maxVal3, maxVal4, maxVal5, maxVal6, maxVal7]

    maxmax = e.pop(random.randint(0, 6))
    ...
    if maxmax == maxVal1:
        ...
        top_left = maxVal7
        best_x = top_left[0] + 1030
        best_y = top_left[1]
        return best_x, best_y
    else:
        top_left = maxVal6
        best_x = top_left[0] + 1270
        best_y = top_left[1] + 320
        return best_x, best_y
...
def filler(image, patch, bs, os, xi, xf, yi, yf):
    num_patch = 0
    stepy = yi
    while stepy < yf:
        stepx = xi
...
def run():
    texture = cv2.imread('Resources/im03.png')
    ...
    res2 = filler(res, texture, batch_size, overlap_size, xstart, xend, ystart, yend)
    ...
    res5 = filler(res4, texture, batch_size, overlap_size, xstart, xend, ystart, yend)
    res6 = last_row_fixer(res5, texture, batch_size, xstart, xend, ystart, yend)
    res7 = last_col_fixer(res6, texture, batch_size, xstart, xend, ystart, yend)
    res7 = last_block(res7, texture, batch_size, xstart, xend, ystart, yend)
    ...
    res8 = filler(res7, texture, batch_size, overlap_size, xstart, xend, ystart, yend)
    res9 = last_row_fixer(res8, texture, batch_size, xstart, xend, ystart, yend)
    res10 = last_col_fixer(res9, texture, batch_size, xstart, xend, ystart, yend)
    res10 = last_block(res10, texture, batch_size, xstart, xend, ystart, yend)
    cv2.imwrite('Result/res15.jpg', res10)
